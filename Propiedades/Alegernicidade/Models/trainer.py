...
class Trainer:
    ...
    def initialize_models(self):
        ...
        pos_seqs, _ = self._load_sequences(self.train_pos, label=1)
        neg_seqs, _ = self._load_sequences(self.train_neg, label=0)
        num_pos = len(pos_seqs)
        num_neg = len(neg_seqs)
        total = num_pos + num_neg

        # Inverse‐frequency weighting: more weight for the minority class
        weight_neg = total / (2 * num_neg)
        weight_pos = total / (2 * num_pos)
        class_weights = torch.tensor(
            [weight_neg, weight_pos],
            device=self.device,
            dtype=torch.float16,        # ← match the AMP dtype
        )
    ...
    def val_epoch(self) -> Dict[str, float]:
        """Calcula perda + métricas (F1, etc.) no conjunto de validação
        e faz o log em wandb no mesmo step global."""
        self.model.eval()
        total_loss = 0
        ema_alpha = 0.97
        ema_loss = None

        # --- loop da perda (igual antes) ---------------------------------
        for ids, mask, y in tqdm(self.val_loader, desc="Val"):
            ...
            # EMA para suavizar o gráfico
            ema_loss = loss_val if ema_loss is None else (
                ema_alpha * ema_loss + (1 - ema_alpha) * loss_val
            )

            wandb.log(
                {
                    "val/loss": loss_val,
                    "val/loss_ema": ema_loss,
                },
                step=self.global_step,
            )

        # --- métricas de classificação -----------------------------------
        metrics = self._compute_metrics(self.val_loader)          # <── aqui
        # faz merge da perda média
        metrics["loss"] = total_loss / len(self.val_loader)

        # loga cada métrica individualmente
        wandb.log({f"val/{k}": v for k, v in metrics.items()}, step=self.global_step)

        return metrics  # agora contém loss + accuracy + f1 + etc.
